...
const UserNutritions = ({UserID}) => {
    ...
    const PostUserNutritionLogs = async (NutritionLog) => {
      ...
      try {
        const response = await axios.post("https://lifetracker-bk1k.onrender.com/nutrition/addnutritiondata", { 'name' :NutritionLog.name, 'calories': NutritionLog.calories, 'category': NutritionLog.category, 'quantity': NutritionLog.quantity, 'url': NutritionLog.url , 'UserID': UserID });
